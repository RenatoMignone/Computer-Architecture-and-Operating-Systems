...
void TIMER0_Handler( void )
{
    ...
    /* Clear interrupt. */
    //whenever there is an interrupt there is a bit in ARM that is set to 1.
    //First so you need to reset that bit. This requires you to know the register associated to the interrupt.
    //we use the datastructure defined in the library. WIthout the library, you need to find the memo address of the specific register where you have to set that bit.
    ...
    //giving the semaphore in the ISR. Instead of using the give, we use the give from ISR, the behaviour is the same but this one is sage, this releases task B so that it can perform the copmutation. We need now to tell the scheduler that might need to switch the task.
    //We call the functino that asks the scheduler to perform a context switch if the parameter we passed is true.

    //if a process is unlocked the parameter will be set to true, so the second parameter is now passed in the other functino. If the semaphore unlocks a task the parameter = True, so we ask the sceduler to context switch. If there is no task, the value is still False.
...
void vInitialiseTimers( void )
{
    ...
    */
    ...
    CMSDK_TIMER0->CTRL     = ( ( 1ul <<  3 ) | /* Enable Timer interrupt. */
        ( 1ul <<  0 ) );  /* Enable counting of the Timer. */
    ...
    */
    ...
    NVIC_cmsdkEnableIRQ( TIMER1_IRQn );
}
