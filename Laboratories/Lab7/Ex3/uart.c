...
void uart_init(Uart_TxRx *uart, void __iomem *base_addr){
	...
	uart -> status_reg = base_addr + 0x18;
	 ...
	 /* Set to clock frequency / deisired baud rate */
    uart -> baudrate_div = base_addr + 0x024;

	/* Line control register*/
    uart -> lcr = base_addr + 0x02C;
}
...
uint32_t uart_tx(Uart_TxRx * uart, const uint8_t *str, uint32_t n_bytes){
    ...
    for (int i = 0; i < n_bytes; i++) {
        ...
        /* If the null-terminator is encountered early, return incomplete
		 * status */
