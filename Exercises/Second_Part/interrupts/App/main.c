...
//we want to implement the deferred interrupts mechanism.
//we need to have 2 tasks. These contants define the sizes, in task A we use more space.
...
//UART to perform Printf, we use the printf more complex than the previous one, to do a full implementation we need a few lines that set the memo addresses of the UART interface.
...
//we cannot use rand() because it needs the malloc.
...
//everytime that there is an interrupt on timer0
void TaskB( void * pvParameters ) {
    ( void ) pvParameters;
    //infinite loop, and the task need to wait on a binary semaphore
    //this task can use the normal semaphore because is not an interrupt. We call the take and if the semaphore is busy the task stops. The semaphore will be released by the interrupt. 
    //We will have 2 interrupts. (in general not herin general not heree)
    for (;;) {
...
void TaskA( void * pvParameters )
{
     ...
     */
        ...
        //this function allows you to set a deadline and takes computes the time that you still need to wait for the next activation.
        //a periodic task in FreeRTOS is an infinire loop
...
void main( void )
{
    ...
    //task A is the main computation that performs the fibonacci num.
    //Task B waits for the semaphore until it is free.
    //In our stratup we configured the microprocessor to execute a function when we have an interrupt, so we just need to implement the function and put it in the MAkeFile
